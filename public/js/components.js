...
function DeadlineItem(deadline) {
    const dateVal     = deadline.end_date || deadline.start_date;
    const itemDate    = dateVal ? new Date(dateVal + (dateVal.includes('T') ? '' : 'T12:00:00')) : null;
    const formattedDate = itemDate
        ? itemDate.toLocaleDateString('es-ES', { day: 'numeric', month: 'long' })
        : 'Fecha pendiente';

    const isFav = deadline.type === 'favorite_video';

    const icon = isFav
        ? `<svg class="w-4 h-4 text-rose-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
    ...
    const iconBg = isFav ? 'bg-rose-900' : 'bg-slate-700';

    return `
        <div class="flex items-center space-x-3">
            <div class="shrink-0 p-2 ${iconBg} rounded-full">
                ${icon}
            </div>
            <div>
